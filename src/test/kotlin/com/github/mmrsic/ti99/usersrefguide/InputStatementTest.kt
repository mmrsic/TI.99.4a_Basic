         ...
         setKeyboardInputProvider(object : KeyboardInputProvider {
            override fun provideInput(ctx: KeyboardInputProvider.InputContext): Sequence<Char> {
               return when (ctx.prompt) {
                  ...
                  "TAX?" -> "500\r"
                  "SALES TAX?" -> "500\r"
                  else -> throw NotImplementedError("Unexpected prompt ${ctx.prompt}")
               }.asSequence()
            }
         })
      }
      ...
      machine.setKeyboardInputProvider(object : KeyboardInputProvider {
         override fun provideInput(ctx: KeyboardInputProvider.InputContext): Sequence<Char> {
            return (when (ctx.programLine) {
               ...
               190 -> "TEXAS"
               else -> throw NotImplementedError("Unexpected program line: ${ctx.programLine}")
            } + "\r").asSequence()
         }
      })
      val interpreter = TiBasicCommandLineInterpreter(machine)
         ...
         setKeyboardInputProvider(object : KeyboardInputProvider {
            override fun provideInput(ctx: KeyboardInputProvider.InputContext): Sequence<Char> {
               return (when (ctx.unacceptedInputs) {
                  ...
                  1 -> "HI,3"
                  2 -> "23,HI"
                  else -> throw NotImplementedError("Code is not prepared to provide input for $ctx")
               } + "\r").asSequence()
            }
         })
      }
