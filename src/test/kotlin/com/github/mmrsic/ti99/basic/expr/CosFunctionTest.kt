...
class CosFunctionTest {
    ...
    fun testSomeRadian() {
        ...
        assertEquals(.5, result.toNative(), "COS($argument) must yield .5")
    }
    ...
    fun testSixtyDegreesInsteadOfRadians() {
        val argument = 60
        val result = CosFunction(NumericConstant(argument)).value()
        assertEquals(-.9524129804, result.toNative(), "COS($argument) must yield -.9524129804")
    }
    ...
    fun test() {
        ...
        assertEquals(.722420351, CosFunction(alpha).value().toNative())
        assertEquals(-.4741676154, CosFunction(beta).value().toNative())
    }
}